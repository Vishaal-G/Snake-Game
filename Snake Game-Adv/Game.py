...
class Game:
    ...
    def startMenu(self):
        ...
        while True:
            ...
            #Create font for title, and for sub-instructions
            pygame.font.init()
            font = pygame.font.Font(Config['game']['title-font'], 38)
            instructions_font = pygame.font.Font(Config['game']['font'], 34)
            
            #Create each message, add the font, then add each rect to the game screen 
            game_text = 'Snake'
            gameTitle = font.render(game_text, True, Config['colors']['white'])

            play_text = 'Space -> Play now'
            play = instructions_font.render(play_text, True, Config['colors']['white'])

            instructions_text = 'I -> Instructions'
            instructions = instructions_font.render(instructions_text, True, Config['colors']['white'])

            quit_text = 'Q -> Quit'
            quitText = instructions_font.render(quit_text, True, Config['colors']['white'])

            gameTitle_rect = gameTitle.get_rect(
                center=(
                    Config['game']['width']/2, 
                    Config['game']['height'] * (1/ 6)
                )
            )
            ...
            instructions_rect = instructions.get_rect(
                center=(
                    Config['game']['width']/2, 
                    Config['game']['height'] * (4/ 6)
                )
            )

            quit_rect = quitText.get_rect(
                center=(
                    Config['game']['width']/2, 
                    Config['game']['height'] * (5/6)
                )
            )


            # Display each message
            self.display.blit(gameTitle, gameTitle_rect)
            self.display.blit(play, play_rect)
            self.display.blit(instructions, instructions_rect)
            self.display.blit(quitText, quit_rect)

            # Update game screen with new display
    ...
    def instructions(self):
        ...
        # Display image
        image = pygame.image.load('./controller1.png')
        self.display.blit(image, (0,0))

        # Update display to instructions screen
        pygame.display.update()
    ...
    def loop(self):
        # Run main menu, if user quits exit the game
        if self.startMenu() != 1:
            exit()
